...
const chartDirectory = path.resolve(defaultDir,"chartiq");
const srcDirectory = path.resolve(defaultDir, "src");
const envi = process.env.NODE_ENV || "production";
module.exports = (env)  => {
	
	console.info(defaultDir, chartDirectory, srcDirectory);
	return {
	entry: "./src/js/sample-template-webpack.js",
	mode: envi,
	module: {
		/**
	...
	plugins: [
		...
		new CopyPlugin({
			patterns: [
				{
					from: path.join(defaultDir, "chartiq/js/thirdparty/html2canvas.min.js"),
					to: "chartiq/js/thirdparty/html2canvas.min.js"
				}
			]
		}),
		/***
	...
	resolve: {
		 ...
		 */
		alias: {
			chartiq: path.resolve(defaultDir, "chartiq")
		}
	}
};

console.info("END OF WEB PACK CONFIG....")

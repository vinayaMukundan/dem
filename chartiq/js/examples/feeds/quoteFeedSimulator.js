...
quoteFeedSimulator.postAjax = function (url, cb) {
	...
	server.onload = function () {
		console.log("RESPONSE TEXT : ",server.responseText)
...
quoteFeedSimulator.fetchInitialData = function (
	...
	console.log("fetch initial interval : ",params.interval)
	var queryUrl =
		...
		console.log("before initial ajax call ")
	...
	quoteFeedSimulator.postAjax(queryUrl, function (status, response) {
		...
		if (status == 200) {
			...
			console.log("inisode initial data")
			cb({
				quotes: newQuotes,
				moreAvailable: true,
...
quoteFeedSimulator.fetchUpdateData = function (symbol, startDate, params, cb) {
	var queryUrl =
		...
		params.period +
		"&extended=" +
		(params.extended ? 1 : 0); // using filter:true for after 
...
quoteFeedSimulator.formatChartData = function (response, symbol) {
	...
	for (var i = 0; i < feeddata.length; i++) {
		console.log("INSIDE FOR LOOP...",feeddata[i])
		var newQuote = {};
		...
		console.log("NEW QUOTE OBJ...",newQuote)
		newQuotes.push(newQuote);
		console.log("NEW QUOTE OBJECTS ARRAY...",newQuotes)
	}
	// console.log("NEW QUOTE OBJECTS ARRAY...",newQuotes)
	return newQuotes;
};
